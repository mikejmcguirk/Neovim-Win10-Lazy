...
local function alloc_labels_start(wins, win_targets, count_labels, labels, ctx)
    ...
    for i = 1, len_wins do
        ...
        local start, stop = get_alloc_iters(upward, len_targets, count_new_labels)
        local count = math.abs(start - stop) + 1

        for label in targets:alloc_start_labels(start, stop, upward, count) do
...
local function alloc_labels_fin(wins, win_targets, count_labels, labels, ctx)
    ...
    for i = 1, len_wins do
        ...
        local count = math.abs(stop - start) + 1

        for label in targets:alloc_fin_labels(start, stop, use_upward, count) do
            labels[#labels + 1] = label
        end
    end
...
local function alloc_labels_cursor(wins, win_targets, count_labels, labels, ctx)
    ...
    for i = 1, len_wins do
        ...
        local targets = win_targets[wins[i]]
        local len_targets = targets:get_no_stat_len()
        local new_labels = math.min(rem_labels, len_targets)
        rem_labels = rem_labels - new_labels
        local s, p = get_alloc_iters(upward, len_targets, new_labels)

        local a_start = 1

        for j, start_row, start_col in targets:iter_no_stat_start_pos(s, p) do
        ...
        local s_stop = a_start - 1

        for label in targets:alloc_start_labels(s, s_stop, upward, new_labels) do
            labels[#labels + 1] = label
        end

        local new_len_labels = #labels
        local added = new_len_labels - old_len_labels
        local to_add = new_labels - added

        for label in targets:alloc_fin_labels(a_start, p, upward, to_add) do
            labels[#labels + 1] = label
        end
    end
...
local function alloc_labels_both(wins, win_targets, count_labels, labels, ctx)
    ...
    for i = 1, len_wins do
        ...
        for label_1, label_2 in targets:alloc_both_labels(start, stop, is_upward, count_new) do
...
---@param ctx farsight.labeler.LabelCtx
---@return boolean
function M.fill_labels(wins, win_targets, tokens, ctx)
    ...
    local ok, labels = alloc_labels(wins, win_targets, tokens, ctx)
    if (not ok) or not labels then
        return false
    end

    populate_labels(labels, tokens, ctx)
    return true
end
...
local function dbg_validate_fill_virt_text(targets)
    ...
    assert(targets:get_no_stat_len() >= 1)
end
