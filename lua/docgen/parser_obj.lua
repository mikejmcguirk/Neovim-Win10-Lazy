...
-- TODO: Re-add (exact) to the class definition once the de-OOPing is done.
...
local function fun_set_class_info_from_class(obj, class_in)
    rawset(obj, "parent", class_in.parent)
    ...
    -- DOC: This behavior.
    if rawget(obj, "classvar") == rawget(obj, "modvar") then
        return
    end

    rawset(obj, "class", class_in.class)

    local class_tag = class_in.tag
    local sep = rawget(obj, "sep")
    local namevar = rawget(obj, "namevar")
    rawset(obj, "tag", class_tag .. sep .. namevar .. "()")

    local see = obj_get_or_create_table_field(obj, "see") ---@type string[]
    see[#see + 1] = "|" .. class_tag .. "|"
end

---@param class docgen.ParserObj
---@param fun docgen.ParserObj Modified in place
function M.class_attach_fun_field(class, fun)
...
---@param obj docgen.ParserObj
---@return integer
function M.fields_count(obj)
    local fields = obj.fields
    return fields and #fields or 0
end
...
---@param obj docgen.ParserObj
---@param parsed nvim.luacats.grammar.Result
function M.add_parsed(obj, parsed)
