...
local lsp_cmds = require("mjm.lsp_cmds").get_lsp_cmds()
...
local function on_lsp_attach(ev)
    ...
    Map("n", "grc", lsp_cmds.in_call, { buffer = buf })

    --- callHierarchy/outgoingCalls ---
    Map("n", "grC", lsp_cmds.out_call, { buffer = buf })

    --- textDocument/codeAction ---
    Map("n", "gra", lsp_cmds.code_action, { buffer = buf })
    ...
    Map("n", "grd", lsp_cmds.declaration, { buffer = buf })
    Map("n", "grD", lsp_cmds.peek_declaration)

    --- textDocument/definition ---
    if client:supports_method("textDocument/definition") then
        Map("n", "gd", lsp_cmds.definition, { buffer = buf })
        Map("n", "gD", lsp_cmds.peek_definition)
    end
    ...
    Map("n", "gO", lsp_cmds.symbols, { buffer = buf })
    ...
    Map("n", "gri", lsp_cmds.implementation)
    Map("n", "grI", lsp_cmds.peek_implementation, { buffer = buf })
    ...
    Map("n", "grr", lsp_cmds.references, { buffer = buf })
    Map("n", "grR", lsp_cmds.peek_references, { buffer = buf })
    ...
    Map("n", "grt", lsp_cmds.typedef, { buffer = buf })
    if client:supports_method("textDocument/typeDefinition") then
        Map("n", "grT", lsp_cmds.peek_typedef, { buffer = buf })
    else
    ...
    Map("n", "gW", lsp_cmds.workspace, { buffer = buf })
