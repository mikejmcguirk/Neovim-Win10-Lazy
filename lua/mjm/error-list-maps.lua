...
-- TODO: Should be consolidated I think into util. The diag module has its own version of this,
-- and will need it for spreading out the other cmd logic. Can probably use for validation in
...
local qflist_new = { use_loclist = false, action = "new" } --- @type QfRancherOutputOpts
local qflist_replace = { use_loclist = false, action = "replace" } --- @type QfRancherOutputOpts
local qflist_add = { use_loclist = false, action = "add" } --- @type QfRancherOutputOpts
local llist_new = { use_loclist = true, action = "new" } --- @type QfRancherOutputOpts
local llist_replace = { use_loclist = true, action = "replace" } --- @type QfRancherOutputOpts
local llist_add = { use_loclist = true, action = "add" } --- @type QfRancherOutputOpts
...
local rancher_keymaps = {
    ...
    { nn, pqfr.. "Qdiags-n-hint",  qp.."in", "All buffer diagnostics min hint"..n,         function() ed.diags("hint", { sev_type = "min" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-n-info",  qp.."if", "All buffer diagnostics min info"..n,         function() ed.diags("info", { sev_type = "min" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-n-warn",  qp.."iw", "All buffer diagnostics min warn"..n,         function() ed.diags("warn", { sev_type = "min" }, { action = "new" , use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-n-error", qp.."ie", "All buffer diagnostics min error"..n,        function() ed.diags("error", { sev_type = "min" }, { action = "new", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-n-top",   qp.."it", "All buffer diagnostics top severity"..n,     function() ed.diags("top", { sev_type = "min" }, { action = "new", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-n-hint",  lp.."in", "Cur buf diagnostics min hint"..n,            function() ed.diags("hint", { sev_type = "min" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-info",  lp.."if", "Cur buf diagnostics min info"..n,            function() ed.diags("info", { sev_type = "min" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-warn",  lp.."iw", "Cur buf diagnostics min warn"..n,            function() ed.diags("warn", { sev_type = "min" }, { action = "new" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-error", lp.."ie", "Cur buf diagnostics min error"..n,           function() ed.diags("error", { sev_type = "min" }, { action = "new", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-top",   lp.."it", "Cur buf diagnostics top severity"..n,        function() ed.diags("top", { sev_type = "min" }, { action = "new", use_loclist = true }) end },

    { nn, pqfr.. "Qdiags-r-hint",  qp.."In", "All buffer diagnostics min hint"..r,         function() ed.diags("hint", { sev_type = "min" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-info",  qp.."If", "All buffer diagnostics min info"..r,         function() ed.diags("info", { sev_type = "min" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-warn",  qp.."Iw", "All buffer diagnostics min warn"..r,         function() ed.diags("warn", { sev_type = "min" }, { action = "replace" , use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-error", qp.."Ie", "All buffer diagnostics min error"..r,        function() ed.diags("error", { sev_type = "min" }, { action = "replace", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-top",   qp.."It", "All buffer diagnostics top severity"..r,     function() ed.diags("top", { sev_type = "min" }, { action = "replace", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-r-hint",  lp.."In", "Cur buf diagnostics min hint"..r,            function() ed.diags("hint", { sev_type = "min" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-info",  lp.."If", "Cur buf diagnostics min info"..r,            function() ed.diags("info", { sev_type = "min" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-warn",  lp.."Iw", "Cur buf diagnostics min warn"..r,            function() ed.diags("warn", { sev_type = "min" }, { action = "replace" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-error", lp.."Ie", "Cur buf diagnostics min error"..r,           function() ed.diags("error", { sev_type = "min" }, { action = "replace", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-top",   lp.."It", "Cur buf diagnostics top severity"..r,        function() ed.diags("top", { sev_type = "min" }, { action = "replace", use_loclist = true }) end },

    { nn, pqfr.. "Qdiags-a-hint",  qp.."<C-i>n", "All buffer diagnostics min hint"..a,     function() ed.diags("hint", { sev_type = "min" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-info",  qp.."<C-i>f", "All buffer diagnostics min info"..a,     function() ed.diags("info", { sev_type = "min" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-warn",  qp.."<C-i>w", "All buffer diagnostics min warn"..a,     function() ed.diags("warn", { sev_type = "min" }, { action = "add" , use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-error", qp.."<C-i>e", "All buffer diagnostics min error"..a,    function() ed.diags("error", { sev_type = "min" }, { action = "add", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-top",   qp.."<C-i>t", "All buffer diagnostics top severity"..a, function() ed.diags("top", { sev_type = "min" }, { action = "add", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-a-hint",  lp.."<C-i>n", "Cur buf diagnostics min hint"..a,        function() ed.diags("hint", { sev_type = "min" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-info",  lp.."<C-i>f", "Cur buf diagnostics min info"..a,        function() ed.diags("info", { sev_type = "min" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-warn",  lp.."<C-i>w", "Cur buf diagnostics min warn"..a,        function() ed.diags("warn", { sev_type = "min" }, { action = "add" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-error", lp.."<C-i>e", "Cur buf diagnostics min error"..a,       function() ed.diags("error", { sev_type = "min" }, { action = "add", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-top",   lp.."<C-i>t", "Cur buf diagnostics top severity"..a,    function() ed.diags("top", { sev_type = "min" }, { action = "add", use_loclist = true }) end },

    { nn, pqfr.. "Qdiags-n-HINT",  qp.."iN", "All buffer diagnostics only hint"..n,        function() ed.diags("hint", { sev_type = "only" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-n-INFO",  qp.."iF", "All buffer diagnostics only info"..n,        function() ed.diags("info", { sev_type = "only" }, { action = "new", use_loclist = false }) end },
    ...
    { nn, pqfr.. "Qdiags-n-ERROR", qp.."iE", "All buffer diagnostics only error"..n,       function() ed.diags("error", { sev_type = "only" }, { action = "new", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-n-TOP",   qp.."iT", "All buffer diagnostics top severity"..n,     function() ed.diags("top", { sev_type = "top" }, { action = "new", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-n-HINT",  lp.."iN", "Cur buf diagnostics only hint"..n,           function() ed.diags("hint", { sev_type = "only" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-INFO",  lp.."iF", "Cur buf diagnostics only info"..n,           function() ed.diags("info", { sev_type = "only" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-WARN",  lp.."iW", "Cur buf diagnostics only warn"..n,           function() ed.diags("warn", { sev_type = "only" }, { action = "new" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-ERROR", lp.."iE", "Cur buf diagnostics only error"..n,          function() ed.diags("error", { sev_type = "only" }, { action = "new", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-TOP",   lp.."iT", "Cur buf diagnostics top severity"..n,        function() ed.diags("top", { sev_type = "top" }, { action = "new", use_loclist = true }) end },

    { nn, pqfr.. "Qdiags-r-HINT",  qp.."IN", "All buffer diagnostics only hint"..r,        function() ed.diags("hint", { sev_type = "only" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-INFO",  qp.."IF", "All buffer diagnostics only info"..r,        function() ed.diags("info", { sev_type = "only" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-WARN",  qp.."IW", "All buffer diagnostics only warn"..r,        function() ed.diags("warn", { sev_type = "only" }, { action = "replace" , use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-ERROR", qp.."IE", "All buffer diagnostics only error"..r,       function() ed.diags("error", { sev_type = "only" }, { action = "replace", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-TOP",   qp.."IT", "All buffer diagnostics top severity"..r,     function() ed.diags("top", { sev_type = "top" }, { action = "replace", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-r-HINT",  lp.."IN", "Cur buf diagnostics only hint"..r,           function() ed.diags("hint", { sev_type = "only" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-INFO",  lp.."IF", "Cur buf diagnostics only info"..r,           function() ed.diags("info", { sev_type = "only" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-WARN",  lp.."IW", "Cur buf diagnostics only warn"..r,           function() ed.diags("warn", { sev_type = "only" }, { action = "replace" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-ERROR", lp.."IE", "Cur buf diagnostics only error"..r,          function() ed.diags("error", { sev_type = "only" }, { action = "replace", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-TOP",   lp.."IT", "Cur buf diagnostics top severity"..r,        function() ed.diags("top", { sev_type = "top" }, { action = "replace", use_loclist = true }) end },

    { nn, pqfr.. "Qdiags-a-HINT",  qp.."<C-i>N", "All buffer diagnostics only hint"..a,    function() ed.diags("hint", { sev_type = "only" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-INFO",  qp.."<C-i>F", "All buffer diagnostics only info"..a,    function() ed.diags("info", { sev_type = "only" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-WARN",  qp.."<C-i>W", "All buffer diagnostics only warn"..a,    function() ed.diags("warn", { sev_type = "only" }, { action = "add" , use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-ERROR", qp.."<C-i>E", "All buffer diagnostics only error"..a,   function() ed.diags("error", { sev_type = "only" }, { action = "add", use_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-TOP",   qp.."<C-i>T", "All buffer diagnostics top severity"..a, function() ed.diags("top", { sev_type = "top" }, { action = "add", use_loclist = false }) end },

    { nn, pqfr.. "Ldiags-a-HINT",  lp.."<C-i>N", "Cur buf diagnostics only hint"..a,       function() ed.diags("hint", { sev_type = "only" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-INFO",  lp.."<C-i>F", "Cur buf diagnostics only info"..a,       function() ed.diags("info", { sev_type = "only" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-WARN",  lp.."<C-i>W", "Cur buf diagnostics only warn"..a,       function() ed.diags("warn", { sev_type = "only" }, { action = "add" , use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-ERROR", lp.."<C-i>E", "Cur buf diagnostics only error"..a,      function() ed.diags("error", { sev_type = "only" }, { action = "add", use_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-TOP",   lp.."<C-i>T", "Cur buf diagnostics top severity"..a,    function() ed.diags("top", { sev_type = "top" }, { action = "add", use_loclist = true }) end },
    ...
    { nn, pqfr.."-qsort-n-fname-asc)",       qp.."tf",  "Qsort by fname asc"..n,           function() et.sort("fname", { dir = "asc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-fname-desc)",      qp.."tF",  "Qsort by fname desc"..n,          function() et.sort("fname", { dir = "desc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-fname-diag-asc)",  qp.."tif", "Qsort by fname_diag asc"..n,      function() et.sort("fname_diag", { dir = "asc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-fname-diag-desc)", qp.."tiF", "Qsort by fname_diag desc"..n,     function() et.sort("fname_diag", { dir = "desc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-severity-asc)",    qp.."tis", "Qsort by severity asc"..n,        function() et.sort("severity", { dir = "asc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-severity-desc)",   qp.."tiS", "Qsort by severity desc"..n,       function() et.sort("severity", { dir = "desc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-type-asc)",        qp.."tt",  "Qsort by type asc"..n,            function() et.sort("type", { dir = "asc" }, { action = "new", use_loclist = false }) end },
    { nn, pqfr.."-qsort-n-type-desc)",       qp.."tT",  "Qsort by type desc"..n,           function() et.sort("type", { dir = "desc" }, { action = "new", use_loclist = false }) end },

    { nn, pqfr.."-qsort-r-fname-asc)",       qp.."Tf",  "Qsort by fname asc"..r,           function() et.sort("fname", { dir = "asc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-fname-desc)",      qp.."TF",  "Qsort by fname desc"..r,          function() et.sort("fname", { dir = "desc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-fname-diag-asc)",  qp.."Tif", "Qsort by fname_diag asc"..r,      function() et.sort("fname_diag", { dir = "asc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-fname-diag-desc)", qp.."TiF", "Qsort by fname_diag desc"..r,     function() et.sort("fname_diag", { dir = "desc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-severity-asc)",    qp.."Tis", "Qsort by severity asc"..r,        function() et.sort("severity", { dir = "asc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-severity-desc)",   qp.."TiS", "Qsort by severity desc"..r,       function() et.sort("severity", { dir = "desc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-type-asc)",        qp.."Tt",  "Qsort by type asc"..r,            function() et.sort("type", { dir = "asc" }, { action = "replace", use_loclist = false }) end },
    { nn, pqfr.."-qsort-r-type-desc)",       qp.."TT",  "Qsort by type desc"..r,           function() et.sort("type", { dir = "desc" }, { action = "replace", use_loclist = false }) end },

    { nn, pqfr.."-qsort-a-fname-asc)",       qp.."<C-t>f",  "Qsort by fname asc"..a,       function() et.sort("fname", { dir = "asc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-fname-desc)",      qp.."<C-t>F",  "Qsort by fname desc"..a,      function() et.sort("fname", { dir = "desc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-fname-diag-asc)",  qp.."<C-t>if", "Qsort by fname_diag asc"..a,  function() et.sort("fname_diag", { dir = "asc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-fname-diag-desc)", qp.."<C-t>iF", "Qsort by fname_diag desc"..a, function() et.sort("fname_diag", { dir = "desc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-severity-asc)",    qp.."<C-t>is", "Qsort by severity asc"..a,    function() et.sort("severity", { dir = "asc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-severity-desc)",   qp.."<C-t>iS", "Qsort by severity desc"..a,   function() et.sort("severity", { dir = "desc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-type-asc)",        qp.."<C-t>t",  "Qsort by type asc"..a,        function() et.sort("type", { dir = "asc" }, { action = "add", use_loclist = false }) end },
    { nn, pqfr.."-qsort-a-type-desc)",       qp.."<C-t>T",  "Qsort by type desc"..a,       function() et.sort("type", { dir = "desc" }, { action = "add", use_loclist = false }) end },

    { nn, pqfr.."-lsort-n-fname-asc)",       lp.."tf",  "Lsort by fname asc"..n,           function() et.sort("fname", { dir = "asc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-fname-desc)",      lp.."tF",  "Lsort by fname desc"..n,          function() et.sort("fname", { dir = "desc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-fname-diag-asc)",  lp.."tif", "Lsort by fname_diag asc"..n,      function() et.sort("fname_diag", { dir = "asc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-fname-diag-desc)", lp.."tiF", "Lsort by fname_diag desc"..n,     function() et.sort("fname_diag", { dir = "desc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-severity-asc)",    lp.."tis", "Lsort by severity asc"..n,        function() et.sort("severity", { dir = "asc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-severity-desc)",   lp.."tiS", "Lsort by severity desc"..n,       function() et.sort("severity", { dir = "desc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-type-asc)",        lp.."tt",  "Lsort by type asc"..n,            function() et.sort("type", { dir = "asc" }, { action = "new", use_loclist = true }) end },
    { nn, pqfr.."-lsort-n-type-desc)",       lp.."tT",  "Lsort by type desc"..n,           function() et.sort("type", { dir = "desc" }, { action = "new", use_loclist = true }) end },

    { nn, pqfr.."-lsort-r-fname-asc)",       lp.."Tf",  "Lsort by fname asc"..r,           function() et.sort("fname", { dir = "asc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-fname-desc)",      lp.."TF",  "Lsort by fname desc"..r,          function() et.sort("fname", { dir = "desc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-fname-diag-asc)",  lp.."Tif", "Lsort by fname_diag asc"..r,      function() et.sort("fname_diag", { dir = "asc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-fname-diag-desc)", lp.."TiF", "Lsort by fname_diag desc"..r,     function() et.sort("fname_diag", { dir = "desc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-severity-asc)",    lp.."Tis", "Lsort by severity asc"..r,        function() et.sort("severity", { dir = "asc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-severity-desc)",   lp.."TiS", "Lsort by severity desc"..r,       function() et.sort("severity", { dir = "desc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-type-asc)",        lp.."Tt",  "Lsort by type asc"..r,            function() et.sort("type", { dir = "asc" }, { action = "replace", use_loclist = true }) end },
    { nn, pqfr.."-lsort-r-type-desc)",       lp.."TT",  "Lsort by type desc"..r,           function() et.sort("type", { dir = "desc" }, { action = "replace", use_loclist = true }) end },

    { nn, pqfr.."-lsort-a-fname-asc)",       lp.."<C-t>f",  "Lsort by fname asc"..a,       function() et.sort("fname", { dir = "asc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-fname-desc)",      lp.."<C-t>F",  "Lsort by fname desc"..a,      function() et.sort("fname", { dir = "desc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-fname-diag-asc)",  lp.."<C-t>if", "Lsort by fname_diag asc"..a,  function() et.sort("fname_diag", { dir = "asc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-fname-diag-desc)", lp.."<C-t>iF", "Lsort by fname_diag desc"..a, function() et.sort("fname_diag", { dir = "desc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-severity-asc)",    lp.."<C-t>is", "Lsort by severity asc"..a,    function() et.sort("severity", { dir = "asc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-severity-desc)",   lp.."<C-t>iS", "Lsort by severity desc"..a,   function() et.sort("severity", { dir = "desc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-type-asc)",        lp.."<C-t>t",  "Lsort by type asc"..a,        function() et.sort("type", { dir = "asc" }, { action = "add", use_loclist = true }) end },
    { nn, pqfr.."-lsort-a-type-desc)",       lp.."<C-t>T",  "Lsort by type desc"..a,       function() et.sort("type", { dir = "desc" }, { action = "add", use_loclist = true }) end },
