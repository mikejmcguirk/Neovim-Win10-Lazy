...
local rancher_keymaps = {
    ...
    { nn, pqfr.. "Qdiags-n-warn",  qp.."iw", "All buffer diagnostics min warn"..n,         function() ed.diags("warn", { sev_type = "min" }, { action = "new" , is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-n-error", qp.."ie", "All buffer diagnostics min error"..n,        function() ed.diags("error", { sev_type = "min" }, { action = "new", is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-n-top",   qp.."it", "All buffer diagnostics top severity"..n,     function() ed.diags("top", { sev_type = "min" }, { action = "new", is_loclist = false }) end },

    { nn, pqfr.. "Ldiags-n-hint",  lp.."in", "Cur win diagnostics min hint"..n,            function() ed.diags("hint", { sev_type = "min" }, { action = "new", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-info",  lp.."if", "Cur win diagnostics min info"..n,            function() ed.diags("info", { sev_type = "min" }, { action = "new", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-warn",  lp.."iw", "Cur win diagnostics min warn"..n,            function() ed.diags("warn", { sev_type = "min" }, { action = "new" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-error", lp.."ie", "Cur win diagnostics min error"..n,           function() ed.diags("error", { sev_type = "min" }, { action = "new", is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-top",   lp.."it", "Cur win diagnostics top severity"..n,        function() ed.diags("top", { sev_type = "min" }, { action = "new", is_loclist = true }) end },

    { nn, pqfr.. "Qdiags-r-hint",  qp.."In", "All buffer diagnostics min hint"..r,         function() ed.diags("hint", { sev_type = "min" }, { action = "replace", is_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-info",  qp.."If", "All buffer diagnostics min info"..r,         function() ed.diags("info", { sev_type = "min" }, { action = "replace", is_loclist = false }) end },
    { nn, pqfr.. "Qdiags-r-warn",  qp.."Iw", "All buffer diagnostics min warn"..r,         function() ed.diags("warn", { sev_type = "min" }, { action = "replace" , is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-error", qp.."Ie", "All buffer diagnostics min error"..r,        function() ed.diags("error", { sev_type = "min" }, { action = "replace", is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-r-top",   qp.."It", "All buffer diagnostics top severity"..r,     function() ed.diags("top", { sev_type = "min" }, { action = "replace", is_loclist = false }) end },

    { nn, pqfr.. "Ldiags-r-hint",  lp.."In", "Cur win diagnostics min hint"..r,            function() ed.diags("hint", { sev_type = "min" }, { action = "replace", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-info",  lp.."If", "Cur win diagnostics min info"..r,            function() ed.diags("info", { sev_type = "min" }, { action = "replace", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-warn",  lp.."Iw", "Cur win diagnostics min warn"..r,            function() ed.diags("warn", { sev_type = "min" }, { action = "replace" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-error", lp.."Ie", "Cur win diagnostics min error"..r,           function() ed.diags("error", { sev_type = "min" }, { action = "replace", is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-top",   lp.."It", "Cur win diagnostics top severity"..r,        function() ed.diags("top", { sev_type = "min" }, { action = "replace", is_loclist = true }) end },

    { nn, pqfr.. "Qdiags-a-hint",  qp.."<C-i>n", "All buffer diagnostics min hint"..a,     function() ed.diags("hint", { sev_type = "min" }, { action = "add", is_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-info",  qp.."<C-i>f", "All buffer diagnostics min info"..a,     function() ed.diags("info", { sev_type = "min" }, { action = "add", is_loclist = false }) end },
    { nn, pqfr.. "Qdiags-a-warn",  qp.."<C-i>w", "All buffer diagnostics min warn"..a,     function() ed.diags("warn", { sev_type = "min" }, { action = "add" , is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-error", qp.."<C-i>e", "All buffer diagnostics min error"..a,    function() ed.diags("error", { sev_type = "min" }, { action = "add", is_loclist = false}) end },
    { nn, pqfr.. "Qdiags-a-top",   qp.."<C-i>t", "All buffer diagnostics top severity"..a, function() ed.diags("top", { sev_type = "min" }, { action = "add", is_loclist = false }) end },

    { nn, pqfr.. "Ldiags-a-hint",  lp.."<C-i>n", "Cur win diagnostics min hint"..a,        function() ed.diags("hint", { sev_type = "min" }, { action = "add", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-info",  lp.."<C-i>f", "Cur win diagnostics min info"..a,        function() ed.diags("info", { sev_type = "min" }, { action = "add", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-warn",  lp.."<C-i>w", "Cur win diagnostics min warn"..a,        function() ed.diags("warn", { sev_type = "min" }, { action = "add" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-error", lp.."<C-i>e", "Cur win diagnostics min error"..a,       function() ed.diags("error", { sev_type = "min" }, { action = "add", is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-top",   lp.."<C-i>t", "Cur win diagnostics top severity"..a,    function() ed.diags("top", { sev_type = "min" }, { action = "add", is_loclist = true }) end },

    { nn, pqfr.. "Qdiags-n-HINT",  qp.."iN", "All buffer diagnostics only hint"..n,        function() ed.diags("hint", { sev_type = "only" }, { action = "new", is_loclist = false }) end },
    ...
    { nn, pqfr.. "Ldiags-n-HINT",  lp.."iN", "Cur win diagnostics only hint"..n,           function() ed.diags("hint", { sev_type = "only" }, { action = "new", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-INFO",  lp.."iF", "Cur win diagnostics only info"..n,           function() ed.diags("info", { sev_type = "only" }, { action = "new", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-n-WARN",  lp.."iW", "Cur win diagnostics only warn"..n,           function() ed.diags("warn", { sev_type = "only" }, { action = "new" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-n-ERROR", lp.."iE", "Cur win diagnostics only error"..n,          function() ed.diags("error", { sev_type = "only" }, { action = "new", is_loclist = true}) end },
    ...
    { nn, pqfr.. "Ldiags-r-HINT",  lp.."IN", "Cur win diagnostics only hint"..r,           function() ed.diags("hint", { sev_type = "only" }, { action = "replace", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-INFO",  lp.."IF", "Cur win diagnostics only info"..r,           function() ed.diags("info", { sev_type = "only" }, { action = "replace", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-r-WARN",  lp.."IW", "Cur win diagnostics only warn"..r,           function() ed.diags("warn", { sev_type = "only" }, { action = "replace" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-r-ERROR", lp.."IE", "Cur win diagnostics only error"..r,          function() ed.diags("error", { sev_type = "only" }, { action = "replace", is_loclist = true}) end },
    ...
    { nn, pqfr.. "Ldiags-a-HINT",  lp.."<C-i>N", "Cur win diagnostics only hint"..a,       function() ed.diags("hint", { sev_type = "only" }, { action = "add", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-INFO",  lp.."<C-i>F", "Cur win diagnostics only info"..a,       function() ed.diags("info", { sev_type = "only" }, { action = "add", is_loclist = true }) end },
    { nn, pqfr.. "Ldiags-a-WARN",  lp.."<C-i>W", "Cur win diagnostics only warn"..a,       function() ed.diags("warn", { sev_type = "only" }, { action = "add" , is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-ERROR", lp.."<C-i>E", "Cur win diagnostics only error"..a,      function() ed.diags("error", { sev_type = "only" }, { action = "add", is_loclist = true}) end },
    { nn, pqfr.. "Ldiags-a-TOP",   lp.."<C-i>T", "Cur win diagnostics top severity"..a,    function() ed.diags("top", { sev_type = "top" }, { action = "add", is_loclist = true }) end },
