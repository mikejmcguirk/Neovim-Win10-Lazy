-- PERF Notes:
-- - My stl actually doesn't do much which is good
-- - The impact of the tabline however is not trivial because of all the fname ops it does
--   Needs to be made event driven
-- - Blink by itself doesn't seem to do much
-- - gitsigns runs hunk calcs after. Does something before. Could look at how to minimize
--   calculation, but not huge deal. Seems to run async
-- - I think everything I'm seeing for ts-autotag is capturing the load step. Barely does
--   anything afterwards
-- - When I just turn on autopairs, it doesn't do much. I would bet the ts_conds are a big part of
--   what causes slowdown there
-- - Semantic tokens are actually a huge performance suck because of the position calculation
--   that does into them
-- - The blink friendly snippets thing I think was seeing the load step again
-- - The Lazydev blink source adds a little. Can argue either way about its necessity. Good for
--   being able to see available modules
-- - As expected Treesitter goes absolutely insane
-- - Cannot reproduct autopairs and autotag doing anything. Maybe was just getting load steps
-- TODO: PERF Steps:
-- - Try to pare down semantic tokens
-- - Make tal event driven
-- - Maybe exit fugitive since it does stuff in the background to track Git state in duplicate
--   with Gitsigns
-- - Should be done anyway, but, restrict when autotag can load
-- - Maybe disable ts_conds in autopairs
-- - Lazyload on InsertEnter might just be silly
-- - In general with the autoplugins though, they don't do a lot on insert enter
-- - *Maybe* disable LazyDev autocomplete source
-- - Try not to use ibl or matchparen
-- - I don't think using the new Treesitter would do anything since my tests/results were
--   from using the default Lua parser with the plugin not pack added
