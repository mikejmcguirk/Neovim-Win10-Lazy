...
; General note - Treesitter Rust queries don't tend to inject into comments well, so semantic
; tokens carry the load where they optimally wouldn't need to
; In a lot of cases, both TS and Tokens need to carry the load. Tokens do not provide a highlight
; for base booleans, but the token is still needed for comment injections. Both TS and tokens are
; needed for properly highlighting module declarations
...
; These are needed to overwrite the operator query
(type_arguments
  [
    "<"
    ">"
